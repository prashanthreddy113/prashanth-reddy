...
public class DashboardController : ControllerBase
{
    ...
    public async Task<ActionResult<DashboardDto>> Get([FromQuery] int? projectId, [FromQuery] int? userId, [FromQuery] int days = 30)
    {
        ...
        var byStatus = (await leads.GroupBy(l => l.Status).Select(g => new { g.Key, Count = g.Count() }).ToListAsync())
            .OrderBy(x => (int)x.Key).Select(x => new CountItem(x.Key.ToString(), StatusLabel(x.Key), x.Count)).ToList();
        foreach (var st in Enum.GetValues<LeadStatus>())
            if (byStatus.All(b => b.Key != st.ToString())) byStatus.Add(new CountItem(st.ToString(), StatusLabel(st), 0));
        ...
        var byInterest = Enumerable.Range(1, 5).Select(i => new CountItem(i.ToString(), InterestLabel(i), interestCounts.FirstOrDefault(x => x.Key == i)?.Count ?? 0)).ToList();
