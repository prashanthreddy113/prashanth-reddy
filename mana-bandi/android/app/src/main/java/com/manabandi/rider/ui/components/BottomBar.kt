...
enum class BottomTab { HOME, RIDES, HELP }

/** Three big tabs: 🏠 Home, 🕒 My rides, 📞 Help. */
@Composable
fun ManaBandiBottomBar(
    ...
    NavigationBar(
        ...
        NavigationBarItem(
            selected = current == BottomTab.RIDES,
            onClick = onRides,
            icon = { Text(text = "🕒", fontSize = 28.sp) },
            label = { Text(text = stringResource(R.string.nav_rides), fontSize = 18.sp) },
            colors = itemColors
        )
        NavigationBarItem(
